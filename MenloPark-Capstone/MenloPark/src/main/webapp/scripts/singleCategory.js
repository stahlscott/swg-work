...
function pagination(postTable, currentStart) {
    $.ajax({
        type: 'GET',
        url: $('#page-context').data("context") + '/category/id/' + $('#category-id').data('category-id') + '/posts/visible/size'
    }).success(function (size, status) {
        var numberOfPages = (size / POSTS_PER_PAGE);
//        if (size % POSTS_PER_PAGE === 0) {
//            numberOfPages--;
//        }
...
$('#postPageModal').on('show.bs.modal', function (event) {
    ...
    $.ajax({
        url: $('#page-context').data("context") + '/post/' + id
